...
import { User, Appointment, Message, Post, Resource, Comment, Doctor } from './types';
import { 
  LineChart, 
  Line, 
  XAxis, 
...
// --- Theme Context ---
const ThemeContext = React.createContext<{ isDarkMode: boolean }>({ isDarkMode: true });
...
const GlassCard = ({ children, className, delay = 0 }: { children: React.ReactNode, className?: string, delay?: number }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "backdrop-blur-xl border rounded-3xl shadow-2xl overflow-hidden transition-all duration-500",
        isDarkMode 
          ? "bg-slate-900/40 border-white/10 shadow-black/20" 
          : "bg-white/70 border-indigo-100 shadow-indigo-100/50",
        className
      )}
    >
      {children}
...
const Button = ({ children, onClick, className, variant = 'primary', icon: Icon, disabled }: any) => {
  ...
  const variants = {
    primary: "bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 text-white hover:shadow-lg hover:shadow-indigo-500/30",
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      className={cn(
        "px-6 py-3 rounded-2xl font-semibold transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed",
...
const LandingPage = ({ onStart, loading }: { onStart: () => void, loading: boolean }) => {
  const { isDarkMode } = React.useContext(ThemeContext);
  return (
    <div className="min-h-[90vh] flex flex-col items-center justify-center px-4 sm:px-6 text-center relative overflow-hidden">
      ...
      <motion.div 
        ...
        className={cn(
          "absolute top-[-10%] left-[-10%] w-[40%] h-[40%] blur-[100px] rounded-full",
      ...
      <motion.div 
        ...
        className={cn(
          "absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] blur-[100px] rounded-full",
          isDarkMode ? "bg-indigo-500/20" : "bg-indigo-200/40"
        )}
      />
      ...
      >
        <div className={cn(
          "inline-flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-md border text-sm font-medium mb-8",
          isDarkMode ? "bg-white/10 border-white/20 text-indigo-300" : "bg-indigo-100/50 border-indigo-200 text-indigo-600"
        )}>
        ...
        <h1 className={cn(
          "text-5xl sm:text-7xl lg:text-8xl font-bold tracking-tight mb-8 leading-[1.1]",
        ...
        <p className={cn(
          "text-lg sm:text-xl mb-12 max-w-2xl mx-auto leading-relaxed",
          isDarkMode ? "text-indigo-100/70" : "text-slate-600"
        )}>
        ...
        <div className="flex flex-col items-center gap-8">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 w-full max-w-md sm:max-w-none">
            <Button onClick={onStart} className="w-full sm:w-auto text-lg px-10 py-4 shadow-xl shadow-indigo-500/20">
              Get Started
...
const LoginPage = ({ 
  ...
  onGoogleLogin: () => void, 
  onEmailAuth: (e: string, p: string, isSignUp: boolean) => void,
  loading: boolean, 
  loginType: 'student' | 'admin', 
  setLoginType: (t: 'student' | 'admin') => void, 
  ...
  return (
    <div className="min-h-[90vh] flex flex-col items-center justify-center px-4 relative overflow-hidden py-10 sm:py-20">
      <motion.div
      ...
      >
        <GlassCard className="p-8 sm:p-10 text-center">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-indigo-500/20 text-white">
            <Heart size={32} />
          </div>
          ...
          <p className={cn("text-sm mb-8", isDarkMode ? "text-indigo-200/60" : "text-slate-500")}>
            {isSignUp ? 'Start your journey with SereneSpace' : 'Continue your journey with SereneSpace'}
          </p>
          ...
          {loginType === 'student' ? (
            <>
              <form onSubmit={handleSubmit} className="space-y-4 mb-8">
                <div className="text-left space-y-1.5">
                  <label className={cn("text-xs font-bold uppercase tracking-wider ml-1", isDarkMode ? "text-indigo-300" : "text-indigo-600")}>Email Address</label>
                  <input 
                    type="email"
                    value={email}
                    ...
                    className={cn(
                      "w-full border rounded-2xl px-5 py-3.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all",
                      isDarkMode 
                        ? "bg-white/5 border-white/10 text-white placeholder-indigo-300/20" 
                        : "bg-slate-50 border-indigo-100 text-slate-900 placeholder-slate-400"
                    )}
                    required
                  />
                </div>
                <div className="text-left space-y-1.5">
                  <label className={cn("text-xs font-bold uppercase tracking-wider ml-1", isDarkMode ? "text-indigo-300" : "text-indigo-600")}>Password</label>
                  <input 
                    type="password"
                    value={password}
                ...
                <Button 
                  className="w-full py-4 text-base font-bold shadow-xl shadow-indigo-500/20 mt-2" 
                  disabled={loading}
                  onClick={() => {}} 
                >
                  {loading ? 'Processing...' : (isSignUp ? 'Sign Up' : 'Login as Student')}
                </Button>
              </form>

              <div className="relative mb-8">
                <div className="absolute inset-0 flex items-center">
                  <div className={cn("w-full border-t", isDarkMode ? "border-white/5" : "border-indigo-100")}></div>
                </div>
                <div className="relative flex justify-center text-[10px] uppercase">
                  <span className={cn("px-4 font-bold tracking-widest", isDarkMode ? "bg-slate-900 text-indigo-300/30" : "bg-white text-indigo-400")}>Or continue with</span>
                </div>
              </div>
            </>
          ) : (
            <div className="mb-8">
              <div className={cn(
                "p-4 rounded-2xl border mb-6 text-sm",
                isDarkMode ? "bg-indigo-500/10 border-indigo-500/20 text-indigo-300" : "bg-indigo-50 border-indigo-100 text-indigo-600"
              )}>
                Administrative access is restricted to verified Google accounts only.
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4">
            <button
              ...
              type="button"
              className={cn(
                "w-full flex items-center justify-center gap-3 px-6 py-3.5 rounded-2xl font-bold transition-all disabled:opacity-50 border shadow-sm",
                loginType === 'admin' 
                  ? "bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-500/20 border-transparent" 
                  : isDarkMode 
                    ? "bg-white text-slate-900 hover:bg-slate-100 border-transparent"
                    : "bg-white text-slate-900 hover:bg-indigo-50 border-indigo-100"
              )}
            >
              <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="w-5 h-5" referrerPolicy="no-referrer" />
              Sign in with Google {loginType === 'admin' && 'as Admin'}
            </button>
          </div>
          ...
          <div className="mt-8 pt-8 border-t border-white/5 flex flex-col gap-4">
            {loginType === 'student' && (
              <button 
                ...
                className="text-indigo-500 hover:text-indigo-400 transition-colors text-xs font-bold"
              >
            ...
            <button 
              ...
              className={cn(
                "mx-auto flex items-center gap-2 px-4 py-2 rounded-full border transition-all text-[10px] font-bold uppercase tracking-wider",
                isDarkMode 
                  ? "bg-white/5 border-white/10 text-indigo-300/60 hover:text-white" 
                  : "bg-indigo-50 border-indigo-100 text-indigo-500 hover:bg-indigo-100"
              )}
            >
              {loginType === 'student' ? <Shield size={12} /> : <UserIcon size={12} />}
          ...
          <button 
            ...
            className="mt-8 text-xs text-indigo-500 hover:text-indigo-400 transition-colors font-medium"
          >
            ← Back to Home
          </button>
        </GlassCard>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {doctors.length === 0 && (
              <p className="col-span-3 text-indigo-300/40 text-center py-10 bg-white/5 rounded-3xl border border-white/10">
...
const PsychoeducationalResourceHub = ({ userRole }: { userRole: string | null }) => {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      if (snapshot.empty) {
        // Fallback to static seeds if DB is empty for the first time
                    ...
                    {deletingResourceId === r.id ? (
                      <div className="flex gap-1">
                        <button 
                          ...
                          className="bg-red-500 text-white text-[10px] px-2 py-1 rounded-md font-bold uppercase"
                        >
                          Confirm
                        </button>
                        <button 
                          onClick={(e) => { e.stopPropagation(); setDeletingResourceId(null); }}
                          className="bg-white/20 text-white text-[10px] px-2 py-1 rounded-md font-bold uppercase"
                        >
                          X
                        </button>
                      </div>
                      ...
                      <button 
                        onClick={(e) => { e.stopPropagation(); setDeletingResourceId(r.id); }}
                        className="p-2 rounded-full bg-red-500/20 text-red-400 hover:bg-red-500 transition-colors"
                      >
                        <X size={14} />
                      </button>
                    )}
...
const CommentSection = ({ postId, user, userLikes, userRole }: { postId: string, user: FirebaseUser, userLikes: Set<string>, userRole: string | null }) => {
            ...
            <div className="flex-1 bg-white/5 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {(userRole === 'admin' || comment.userId === user.uid) && (
                    <div className="flex items-center gap-2">
                      {deletingCommentId === comment.id ? (
                        <div className="flex items-center gap-2">
                           <button 
                             onClick={() => handleDeleteComment(comment.id)}
                             className="text-[10px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded-md font-bold uppercase transition-all hover:bg-red-500/30"
                           >
                             Confirm
                           </button>
                           <button 
                             onClick={() => setDeletingCommentId(null)}
                             className="text-[10px] bg-white/10 text-white/60 px-2 py-0.5 rounded-md font-bold uppercase transition-all hover:bg-white/20"
                           >
                             Cancel
                           </button>
                        </div>
...
const ForumPage = ({ user, userRole }: { user: FirebaseUser, userRole: string | null }) => {
  ...
  const handleDeletePost = async (postId: string) => {
    try {
      ...
      // Optionally delete associated comments if needed, but for now just the post
    } catch (error) {
      console.error("Deletion failed:", error);
          ...
          >
            <GlassCard className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {(userRole === 'admin' || post.userId === user.uid) && (
                  <div className="flex items-center gap-2">
                    {deletingPostId === post.id ? (
                      <div className="flex items-center gap-2 p-1 bg-white/5 rounded-xl">
                        <button 
                          ...
                          className="px-3 py-1 bg-red-500 text-white text-[10px] font-bold uppercase rounded-lg hover:bg-red-600 transition-all shadow-lg shadow-red-500/20"
                        >
                          Confirm
                        </button>
                        <button 
                          ...
                          className="px-3 py-1 bg-white/10 text-white/60 text-[10px] font-bold uppercase rounded-lg hover:bg-white/20 transition-all font-mono"
                        >
                          Cancel
                        </button>
                      </div>
                      ...
                      <button 
                        ...
                        className="relative z-10 p-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 active:scale-95 transition-all"
              ...
              {activePostId === post.id && (
                <CommentSection postId={post.id} user={user} userLikes={userLikes} userRole={userRole} />
              )}
            </GlassCard>
          </motion.div>
        ))}
...
const DoctorManagementSection = () => {
  ...
  const handleAddDoctor = async () => {
    ...
    try {
      await addDoc(collection(db, 'doctors'), {
        ...newDoctor,
        image: newDoctor.image || `https://picsum.photos/seed/${newDoctor.name}/100`,
        createdAt: new Date().toISOString()
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <Button onClick={() => setIsAdding(!isAdding)} className="flex items-center gap-2 bg-indigo-500/20 text-indigo-400 border border-indigo-500/50 hover:bg-indigo-500 hover:text-white px-4 py-2">
        ...
        >
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <input 
              ...
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            />
            <input 
              placeholder="Specialty (e.g. Stress, Anxiety)"
          ...
          <Button onClick={handleAddDoctor} className="w-full sm:w-auto px-10">Save Doctor</Button>
        </motion.div>
      )}

      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {doctors.map(doctor => (
          <div key={doctor.id} className="p-4 rounded-2xl bg-white/5 border border-white/10 group relative hover:border-indigo-500/50 transition-all">
            <button 
              onClick={() => handleDeleteDoctor(doctor.id)}
              className="absolute top-2 right-2 p-2 rounded-lg bg-red-500/20 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 hover:text-white"
            >
              <Trash2 size={14} />
            </button>
            <div className="flex items-center gap-3">
              <img src={doctor.image} alt="" className="w-12 h-12 rounded-xl object-cover" referrerPolicy="no-referrer" />
              <div>
                <div className="text-white font-bold text-sm">{doctor.name}</div>
                <div className="text-indigo-300/60 text-[10px] uppercase tracking-wider">{doctor.specialty}</div>
              </div>
            </div>
          </div>
        ))}
        {doctors.length === 0 && !isAdding && (
          <div className="col-span-full py-10 text-center text-indigo-300/20 italic bg-white/5 rounded-2xl border border-dashed border-white/10">
            No doctors listed. Add one to show up for students.
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  const [isSignUpLoginPage, setIsSignUpLoginPage] = useState(false);
  ...
  const syncUserProfile = async (u: FirebaseUser) => {
    try {
      ...
      // Only assign admin role if email matches AND is verified (Google Login ensures this)
      if (u.email === 'rakshakshunashimarad@gmail.com' && u.emailVerified) {
        role = 'admin';
      }
      ...
      if (!userSnap.exists()) {
        ...
        // If it's the admin email, always treat as admin if verified, otherwise use stored role
        return (u.email === 'rakshakshunashimarad@gmail.com' && u.emailVerified) ? 'admin' : (userData.role || 'student');
      }
    } catch (error: any) {
      handleFirestoreError(error, 'syncUserProfile', `users/${u.uid}`);
      // Fallback
      return u.email === 'rakshakshunashimarad@gmail.com' && u.emailVerified ? 'admin' : 'student';
    }
  };
  ...
  useEffect(() => {
    ...
    const authUnsubscribe = onAuthStateChanged(auth, async (u) => {
      if (u) {
        setUser(u);
        const resolvedRole = await syncUserProfile(u);
      ...
      if (u && !isRegisteringRef.current && (currentPage === 'landing' || currentPage === 'login')) {
        const targetPage = (u.email === 'rakshakshunashimarad@gmail.com' && u.emailVerified) ? 'admin-dashboard' : 'dashboard';
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Ensure profile is synced BEFORE potentially signing out
      ...
      if (additionalInfo?.isNewUser) {
        ...
        await signOut(auth);
        alert("New account created successfully with Google! Now please click the Google button again to log in.");
        isRegisteringRef.current = false;
        setIsRegisteringState(false);
        return;
      }
      
      if (loginType === 'admin' && u.email !== 'rakshakshunashimarad@gmail.com') {
        await signOut(auth);
        alert("Access Denied: You are not an authorized administrator.");
        return;
      }

      if (loginType === 'student' && u.email === 'rakshakshunashimarad@gmail.com') {
        await signOut(auth);
        alert("Access Denied: Administrators must use the Admin Login.");
        return;
      }
    } catch (error: any) {
  ...
  const handleEmailAuth = async (email: string, pass: string, isSignUp: boolean) => {
    if (isLoggingIn || loginType === 'admin') return;
    setIsLoggingIn(true);
    if (isSignUp) {
      isRegisteringRef.current = true;
      setIsRegisteringState(true);
    ...
    try {
      if (isSignUp) {
        ...
        // Ensure profile is created BEFORE sign out
        await syncUserProfile(result.user);
        await signOut(auth);
        alert("Account created successfully! Use your new credentials to log in.");
        setIsSignUpLoginPage(false);
      } else {
  ...
  return (
    <ThemeContext.Provider value={{ isDarkMode }}>
      <ErrorBoundary>
        <div className={cn(
          "min-h-screen transition-all duration-700 ease-in-out font-sans",
          isDarkMode 
          ...
          <motion.nav 
            ...
            className="fixed top-0 w-full z-50 px-4 sm:px-6 py-4"
          >
            <div className={cn(
              "max-w-7xl mx-auto flex items-center justify-between backdrop-blur-2xl border rounded-[2rem] px-4 sm:px-8 py-2.5 transition-all duration-500",
              isDarkMode 
                ? "bg-slate-900/50 border-white/10 shadow-2xl shadow-black/20" 
                : "bg-white/70 border-indigo-100 shadow-xl shadow-indigo-100/30"
            )}>
              <div className="flex items-center gap-2 sm:gap-3 cursor-pointer group" onClick={() => setCurrentPage('landing')}>
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-lg shadow-indigo-500/20 group-hover:scale-110 transition-transform">
                  <Heart size={20} className="text-white sm:size-24" />
                </div>
                <span className={cn(
                  "text-xl sm:text-2xl font-black tracking-tighter leading-none",
                  isDarkMode ? "text-white" : "text-slate-900"
                )}>
                  Serene<span className="text-indigo-500">Space</span>
                </span>
              </div>
              ...
              <div className="hidden lg:flex items-center gap-1">
                {user && navItems.map(item => (
                  <button
                    ...
                    className={cn(
                      "px-3 py-2 rounded-xl text-[13px] font-bold transition-all flex items-center gap-2",
                      currentPage === item.id 
              ...
              <div className="flex items-center gap-2 sm:gap-4">
                {/* Language Toggle - Scrollable for mobile if needed */}
                <div className={cn(
                  "hidden md:flex items-center gap-2 rounded-xl px-3 py-1.5 border transition-all",
                  isDarkMode ? "bg-white/5 border-white/10" : "bg-indigo-50 border-indigo-100"
                )}>
                  ...
                  <select 
                    ...
                    className={cn(
                      "bg-transparent text-[11px] font-bold focus:outline-none cursor-pointer uppercase tracking-wider",
                      isDarkMode ? "text-indigo-200" : "text-indigo-600"
                    )}
                  >
                    {languages.map(l => <option key={l} value={l} className={isDarkMode ? "bg-[#1e293b]" : "bg-white"}>{l}</option>)}
                  </select>
                </div>

                {user ? (
                  <div className="flex items-center gap-3">
                    <img src={user.photoURL || undefined} alt="User" className="w-9 h-9 rounded-full border-2 border-indigo-500/50 shadow-sm" referrerPolicy="no-referrer" />
                    <button onClick={handleLogout} className="hidden lg:flex p-2.5 rounded-2xl text-red-400 hover:bg-red-400/10 transition-all">
                      <LogOut size={20} />
                    </button>
                  </div>
                ) : (
                ...
                <button 
                  className={cn(
                    "lg:hidden p-2.5 rounded-2xl transition-all",
                    isDarkMode ? "text-white bg-white/5" : "text-slate-900 bg-slate-100"
                  )}
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <motion.div
                ...
                className={cn(
                  "fixed inset-0 z-40 pt-28 pb-10 px-6 lg:hidden overflow-y-auto",
                  isDarkMode ? "bg-slate-950/98" : "bg-white/98"
                )}
              >
              <div className="flex flex-col gap-3 max-w-lg mx-auto pb-10">
                  {user && navItems.map(item => (
                    <button
                      key={item.id}
                      onClick={() => {
                      ...
                      className={cn(
                        "p-5 rounded-3xl text-lg font-black flex items-center gap-4 transition-all",
                        currentPage === item.id 
                  ...
                  {user && (
                    <button onClick={handleLogout} className="mt-4 p-5 rounded-3xl bg-red-500/10 text-red-500 text-lg font-black flex items-center gap-4 transition-all hover:bg-red-500/20">
                  ...
                  <div className={cn(
                    "mt-4 p-5 rounded-3xl flex items-center justify-between",
                    isDarkMode ? "bg-white/5" : "bg-slate-100"
                  )}>
                    <div className="flex items-center gap-4 font-black text-indigo-500">
                      <Languages size={24} />
                      Language
                    </div>
                    <select 
                      ...
                      onChange={(e) => setLanguage(e.target.value)}
                      className={cn(
                        "bg-transparent font-bold focus:outline-none cursor-pointer uppercase text-sm",
                        isDarkMode ? "text-white" : "text-slate-800"
                      )}
                    >
                      {languages.map(l => <option key={l} value={l} className="bg-slate-800 text-white">{l}</option>)}
                    </select>
                  </div>
                </div>
              </motion.div>
          ...
          {/* Main Content */}
          <main className="pt-28 pb-12 px-4 sm:px-6 min-h-screen relative overflow-hidden">
              ...
              >
                ...
                {currentPage === 'login' && (
                  <LoginPage 
                    ...
                    isSignUp={isSignUpLoginPage}
                    setIsSignUp={setIsSignUpLoginPage}
                  />
                )}
                {currentPage === 'admin-dashboard' && user && userRole === 'admin' && <AdminDashboardPage user={user} userRole={userRole} />}
                {currentPage === 'chat' && user && userRole === 'student' && <ChatPage user={user} language={language} />}
                {currentPage === 'booking' && user && userRole === 'student' && <BookingPage user={user} />}
                {currentPage === 'journal' && user && userRole === 'student' && <JournalPage user={user} />}
                {currentPage === 'quizzes' && user && userRole === 'student' && <QuizzesForReliefPage user={user} />}
                {currentPage === 'resources' && (userRole === 'student' || userRole === 'admin') && <PsychoeducationalResourceHub userRole={userRole} />}
                {currentPage === 'forum' && user && (userRole === 'student' || userRole === 'admin') && <ForumPage user={user} userRole={userRole} />}
                {currentPage === 'dashboard' && user && userRole === 'student' && <DashboardPage user={user} setCurrentPage={setCurrentPage} />}
                {currentPage === 'emergency' && userRole === 'student' && <EmergencyPage />}
              </motion.div>
            </AnimatePresence>
          </main>
          ...
          {/* Footer */}
          <footer className={cn(
            "border-t pt-16 pb-12 px-4 sm:px-6 transition-colors duration-500",
            ...
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-10">
              <div className="flex flex-col items-center md:items-start gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xl font-bold tracking-tight">SereneSpace</span>
                </div>
                <p className={cn("text-sm max-w-xs text-center md:text-left", isDarkMode ? "text-indigo-200/40" : "text-slate-500")}>
                  A digital sanctuary for student mental health and shared growth.
                </p>
              </div>
              
              
              <div className={cn("text-xs font-medium", isDarkMode ? "text-indigo-300/30" : "text-slate-400")}>
                © 2026 SereneSpace. All rights reserved.
              </div>
            </div>
          </footer>
        </div>
